...
> **LLM authoring:** a condensed LLM-facing version of this specification is available at [spec/v1.0-ai.md](./v1.0-ai.md). Use it in place of this file when loading the spec into an AI prompt — it covers 100% of v1.0 normative surface in ~1/3 the size.
...
> **NOTE: Normative Scope. This specification defines the StemJSON v1.0 language. All sections are normative v1.0.**
...
| Context key    | Type      | Required | Description                             |
| -------------- | --------- | -------- | --------------------------------------- |
| _value / value           | state ref   | REQUIRED | Two-way binding to a numeric state key.            |
| _min / min               | number      | Optional | Minimum value. Default: 0.                         |
| _max / max               | number      | Optional | Maximum value. Default: 1.                         |
| _step / step             | number      | Optional | Step increment.                                    |
...
> **ℹ Note: State ownership and propagation. Each module owns its declared state keys exclusively. A state action dispatched from within a module applies a partial-merge update to that module's declared state keys. The update then propagates upward through the component hierarchy: for each ancestor module that has also declared a state key of identical name, the ancestor's key is updated by partial merge with the same value (name-matched upward propagation). Ancestors that have not declared a matching-name key are unaffected. Parent modules cannot write to child-module state; sibling modules cannot write to each other's state; and no module can read another module's state — state reads are strictly scoped to the module that declares the key.**
...
| --------- | ------- | ------------- |
| read      | Open the native photo picker. On success, the output context key receives the selected photo URL (single) or array of URLs (multiple). | `selectionLimit` (number, default 1): maximum photos the user can select. `mediaTypes` (array of strings): filter by `"image"` and/or `"video"`. |
...
| `[field ~ val]`  | Filter predicate (array only). See §8.4 for contains/filter semantics.                                 | `${web[title ~ ${search}]}`   |
...
**Filter-predicate location & ref support.** The `[field ~ val]` filter predicate is a **path-only** construct: it is recognized inside `${…}` / `@{…}` path segments, NOT as a postfix on an arbitrary expression. `${devices}[room ~ "living"]` (outside the `${}`) is parsed as an *expression* subscript and will NOT filter — see §8.3. Always write filters inside the path: `${devices[room ~ "living"]}`.
...
| ---------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| #{now}           | number | Current Unix timestamp in seconds. Updates per render cycle.                                                                                            |
...
| #{today}         | number | Unix timestamp of midnight at the start of today.                                                                                                       |
| #{yesterday}     | number | Unix timestamp of midnight at the start of yesterday.                                                                                                   |
| #{tomorrow}      | number | Unix timestamp of midnight at the start of tomorrow.                                                                                                    |
| #{endofweek}     | number | Unix timestamp of the start of the last day of the current week.                                                                                        |
| #{startofmonth}  | number | Unix timestamp of the first day of the current month at midnight.                                                                                       |
| #{endofmonth}    | number | Unix timestamp of the last second of the current month (23:59:59).                                                                                      |
...
| Form             | Example                                              | Notes                                                                                                          |
| ---------------- | ---------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| System name      | "primary"                                            | Named adaptive colors: primary, secondary, blue, red, green, orange, yellow, white, black, clear, accentColor. |
...
| Expression       | "{{ ${isError} ? 'red' : 'green' }}"                 | Computed color from an expression.                                                                             |
...
| Gradient              | `"background": { "gradient": { "linear": { ... } } }`                                      |
...
| stroke   | object                  | Stroke: { "content": { "color": "..." }, "style": { "lineWidth": number } }. |
...
| replace() | replace(s, from, to) | Replaces all occurrences of from with to. Works on strings and arrays.      |
...
| cast()      | cast(v, type)               | Casts value v to: int | double | float | number | string | str | bool | boolean | expression | expr | date. |
...
> ```
>
> First action captures the outer-scope values (`@{item}` = button's row here) into module state; second action, chained via `output.success`, runs `map()` — inside which `@{item}` now safely rebinds to the array element without conflict, and the target values are read via `${targetId}` state refs.
...
When evaluating `${key}`, the runtime resolves the reference within the current module's declared state only. If the key is not declared in the current module, the reference resolves to `.none` (null). There is no cross-module read resolution: parent, child, and sibling modules' state are not accessible from outside the declaring module. Context references `@{key}` resolve only within the current component's context chain — they do not traverse module boundaries.
...
| onAppear    | array  | Component appears on screen.                                       |
| onDisappear | array  | Component is removed from the visible hierarchy.                   |
...
**Tip for complex lookups.** Prefer sending the full data you need in the trigger payload (e.g. include the display name alongside the id) rather than trying to look up the record inside the handler. Filter-predicate paths with `@{…}` refs do NOT work (see §6.2.1) — so looking up by id forces a two-step state-capture chain via `output.success`, which is tolerable but harder to read.
...
The state action applies a partial update to the current module's state dictionary. Only known keys are updated; unknown keys are silently ignored. After the local update, the update propagates upward by name match: every ancestor module that has also declared a state key of the same name receives the same partial-merge update (see §11.3). Unless overridden by a chained output, this is the default behavior.


| Input form           | Example                              | Meaning                                                            |
| -------------------- | ------------------------------------ | ------------------------------------------------------------------ |
| Dictionary           | "input": { "count": 5, "error": "" } | Update each listed state key with the corresponding value.         |
| String — action id   | "input": "some_action_id"            | Assign the entire result of that action to the matching state key. |
| String — context ref | "input": "@{some_context_key}"       | Assign the resolved context value to the matching state key.       |
...
```json
{ "state": { "id": "set_results", "input": { "results": "@{load_action}", "error": "" } } }

{ "state": { "id": "toggle_secure",
"input": { "isSecured": "{{ !${isSecured} }}" },
"output": {
"success": [
{ "state": { "id": "update_icon",
"input": { "secureIcon": "{{ ${isSecured} ? 'sf://eye.slash' : 'sf://eye' }}" } } }
]
}
} }
```
...
| Operation | Typical params                                                     |
| --------- | ------------------------------------------------------------------ |
| read      | { "id": "doc_id" } or { "collection": "name", "documentId": "id" } |
| create    | { "id": "doc_id", "body": { ... }, "rewrite": true }               |
...
| Operation | Typical params                                     |
| --------- | -------------------------------------------------- |
| read      | params: "key_name" (plain string, the key to read) |
| create    | { "key": "key_name", "value": "${apiKey}" }        |
...
State actions produce no result value. `@{actionId}` is not populated. `output.success` fires after the state update is applied; `output.failure` is never triggered by state actions.
...
State reads are scoped strictly to the module that declares the key. `${key}` resolves only within the current module's state; if the key is not declared in the current module, the reference resolves to `.none` (null). A module cannot read state declared by its parent, child, or sibling modules. Context references (`@{key}`) similarly do not cross module boundaries.
...
- When a component inside a module dispatches a state action, the action updates the state of the enclosing module for any keys that module has declared (partial merge; non-declared keys are ignored).
- **Name-matched upward propagation.** After the local update, the runtime propagates the update upward through the component hierarchy. For each ancestor module that has also declared a state key with the same name as one being updated, that ancestor's state key is updated by partial merge with the same value. An ancestor that has not declared a matching-name key is unaffected. The ancestor "opts in" to receiving the update by declaring a key of identical name. Propagation traverses the full ancestor chain — any ancestor at any depth with a matching declaration receives the update. There is no downward or sibling propagation.
- If the state action has chained output actions (output.success or output.failure defined), it does NOT propagate upward — it stays scoped to the dispatching component. Use chained output to keep updates local.
...
In production the `isLoading` key is typically set to `true` by a `state` action before a `repo` call, then reset to `false` in the `output.success` / `output.failure` chain.
