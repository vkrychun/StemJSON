...
Calls an AI provider through a `remote` repository; provider-agnostic — the request shape lives in `body`, and the API key is injected by the repo's interceptor (never in JSON).

| Field | Required | Value |
|---|---|---|
| `input.provider` | ✓ | id of a `remote` repository dependency (AI endpoint + auth interceptor) |
